/**
 * A flexible random number utility based on xorshift128 and
 * djb2 hash (so that it accepts an arbitrary length string as seed).
 *
 * const prng = PRNG('myOptionalSeed')
 * const r = prng.random() // 0..1
 * const r0 = prng.range(5, 10); // floating point
 * const r1 = prng.rangeFloor(0, length); // as int (length is excluded)
 *
 * To generate a pure random (non-deterministic) seed:
 *
